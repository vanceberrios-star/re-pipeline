...
async function saveRE(clients) {
  try {
    for (const c of clients) {
      await supabase.from("re_clients").upsert({
        id: c.id, name: c.name, phone: c.phone, email: c.email,
        address: c.address, client_type: c.clientType,
        budget: parseFloat(c.budget)||null, property_type: c.propertyType,
        status: c.status, source: c.source, timeline: c.timeline,
        pre_approval: c.preApproval, pre_approval_amt: parseFloat(c.preApprovalAmt)||null,
        motivation: c.motivation, comm_pref: c.commPref,
        last_contact: c.lastContact||null, follow_up: c.followUp||null,
        cross_sell_status: c.crossSellStatus, commission_pct: parseFloat(c.commissionPct)||3,
        closing_date: c.closingDate||null, offer_count: parseInt(c.offerCount)||0,
        homes_shown: parseInt(c.homesShown)||0, referral_person: c.referralPerson,
        attorney: c.attorney, star: c.star||3, notes: c.notes,
        follow_up_attempts: parseInt(c.followUpAttempts)||0,
        activity_log: c.activityLog||[],
      });
    }
  } catch(e) { console.error("saveRE error:", e); }
}
...
export default function REApp() {
  ...
  useEffect(()=>{ if(!loading) saveRE(clients); },[clients]);
        ...
        <div style={{padding:"16px"}}>
          ...
          {!crossLoading&&crossOpps.length>0&&(
            <div>
              ...
              {crossOpps.map((opp,i)=>(
                <CrossCard key={i} opp={opp} expanded={expandedCross===i} onToggle={()=>setExpandedCross(expandedCross===i?null:i)} copied={copied} onCopy={copyText} onStatusChange={v=>updateField(opp.client.id,"crossSellStatus",v)}/>
              ))}
            </div>
          )}
